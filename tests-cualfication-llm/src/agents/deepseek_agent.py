...
class DeepseekAgent(BaseAgent):
    """
    Agent for DeepSeek Chat Completions compatible endpoints via LiteLLM-style or OpenAI-compatible schema.
    Config keys:
      - base_url (required if not using hosted lite endpoints)
      - model (required)
      - api_key (optional; fallback to env DEEPSEEK_API_KEY)
      - parameters (optional): temperature, top_p, max_tokens, etc.
    """
    def infer(self, prompt: str, timeout: float) -> Dict[str, Any]:
        base_url = self.config.get("base_url") or os.environ.get("DEEPSEEK_BASE_URL", "https://api.deepseek.com/v1/chat/completions")
        params = self.config.get("parameters", {})
        model = self.config.get("model") or params.get("model") or os.environ.get("DEEPSEEK_MODEL", "deepseek-chat")
